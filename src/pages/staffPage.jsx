...
const staffPage = () => {
  ...
  const refreshTables = async (query = {}) => {
    ...
    try {
      ...
      if (
        !currentOrdersTable &&
        (Array.isArray(data) ? data.length : (data?.tables || []).length)
      ) {
  ...
  const refreshOrdersForTable = async (table) => {
    ...
    setErrorMessage("");
    try {
      const res = await getBookingsByTable(
        table.id || table._id || table.tableNumber,
        {},
        token
      );
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    ...
    fetchOrders(); // gọi lại danh sách để cập nhật UI
  };
  ...
  const changeTableStatus = async (
    ...
    try {
      ...
      if (
        currentOrdersTable &&
        String(currentOrdersTable.tableNumber) === String(tableNumber)
      ) {
    ...
    if (region) {
      try {
        ...
        if (regionRestaurants.length > 0) {
          ...
          refreshTables({
            restaurantId: regionRestaurants[0]._id || regionRestaurants[0].id,
          });
        }
      } catch (e) {
          ...
          <div className="mt-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
            <div className="flex items-center gap-2 text-blue-800">
              ...
              <span className="font-medium">
                Đang xem: {selectedRestaurant.name} - {selectedDate}
              </span>
            </div>
          </div>
        )}
            ...
            <div className="bg-white border rounded-lg p-4">
              ...
              <div className="space-y-2">
                {order.items.map((item, idx) => (
                  <div
                    key={idx}
                    className="flex justify-between items-start text-xs pb-2 border-b"
                  >
          ...
          <div className="p-4 space-y-4">
            <div className="bg-gray-50 p-3 rounded-lg">
              ...
              <div className="font-bold text-lg">
                {
                  (
                    tableStatusConfig[selectedTable.status] ||
                    tableStatusConfig["available"]
                  ).label
                }
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="border-t pt-3">
                ...
                <p className="text-sm text-gray-600 mb-3">
                  Sử dụng khi khách gọi điện hoặc đến trực tiếp đặt bàn
                </p>
                <input
                  type="text"
              ...
              <div className="border-t pt-3">
                ...
                <p className="text-sm text-gray-600 mb-3">
                  Sử dụng khi bàn cần bảo trì hoặc có vấn đề
                </p>
                <button
                  onClick={() => {
                    changeTableStatus(
          ...
          <div className="p-4 space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <div
                  className={`inline-block ${config.color} text-white px-3 py-1 rounded-full text-xs font-medium mt-1`}
                >
                  {config.label}
                </div>
              </div>
